/* SerializableDictionary.cs - C's Utils
 * 
 * A dictionary which is serialized by unity as a list.
 * 
 * 
...
namespace CsUtils
{
    ...
    public class SerializableDictionary<TKey, TValue> : IDictionary<TKey, TValue>, ISerializationCallbackReceiver
    {
        ...
        private Dictionary<TKey, uint> MakeKeyPositions()
        {
            ...
            {
                result[dictionary[i].Key] = (uint)i;
            }
        ...
        {
            get => dictionary[(int)KeyPositions[key]].Value;
            set
            {
                if (KeyPositions.TryGetValue(key, out uint index))
                {
                    dictionary[(int)index].SetValue(value);
                }
                else
                {
        ...
        public ICollection<TKey> Keys => dictionary.Select(tuple => tuple.Key).ToArray();
        public ICollection<TValue> Values => dictionary.Select(tuple => tuple.Value).ToArray();
        ...
        public bool TryGetValue(TKey key, out TValue value)
        {
            if (KeyPositions.TryGetValue(key, out uint index))
            {
                value = dictionary[(int)index].Value;
